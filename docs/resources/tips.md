# GM Tips & Best Practices

Guidance and tips for running effective D&D games.

<!-- vim-markdown-toc GFM -->

* [Campaign Organization](#campaign-organization)
    * [Campaign Planner](#campaign-planner)
    * [Character Notes](#character-notes)
    * [Player Handouts](#player-handouts)
    * [Adventure Log](#adventure-log)
    * [NPC Notes](#npc-notes)
    * [Campaign Calendar](#campaign-calendar)
    * [Toolbox](#toolbox)
* [Starting the Game](#starting-the-game)
    * [Handling Recaps](#handling-recaps)
    * [Setting the Stage](#setting-the-stage)
* [Combat](#combat)
    * [Combat Guidelines](#combat-guidelines)
    * [Theater of the Mind](#theater-of-the-mind)
    * [Boss Fight Tips](#boss-fight-tips)

<!-- vim-markdown-toc -->

## Campaign Organization

### Campaign Planner
Write down the main story arc of your campaign, and keep track of things that you hope appear in future adventures. Update it as the campaign develops, adding ideas as they come to you.

### Character Notes
Write down the characters' backgrounds and goals, since these notes can help you design adventure content that provides opportunities for character development.

Keep a running tally of the adventurers' classes and levels, as well as any quests and downtime activities they're engaged in. If the characters have a ship or stronghold, record its name and whereabouts, as well as any hirelings in the characters' employ.

### Player Handouts
Keep a copy of all handouts you make for your players so that you don't have to remember their contents later.

### Adventure Log
Think of this log as an episode guide for your campaign. Summarize each game session or adventure to help you keep track of the unfolding campaign story. You can give your players access to this log as well, or to an edited version stripped of your notes and secrets. (The players might also keep their own record of adventures, which you can refer to if your own log is incomplete.)

### NPC Notes
Record statistics and roleplaying notes for any NPC the characters interact with more than once. For example, your notes might differentiate important people in a town by their different voices, as well as their names, the places where they live and work, the names of their family members and associates, and maybe even a secret that each one of them has.

### Campaign Calendar
Your world feels more real to your players when the characters notice the passage of time. Note details such as the change of seasons and major holidays, and keep track of any important events that affect the larger story.

### Toolbox
Keep notes whenever you create or significantly alter a monster, magic item, or trap. Keep any maps, random dungeons, or encounters you create. This information ensures you won't repeat your work, and you'll be able to draw on this material later.

## Starting the Game

### Handling Recaps
Forego the traditional recap in favor of _"When last we left our heroes..."_

If players do want a recap, encourage one of the players to give the recap. This gives you as a DM additional information about how the players may have perceived events differently. It also allows you to create different improvisations in the story based upon something your player has said.

### Setting the Stage
Describe the sights, sounds, and smells of the environment. It is helpful to use at least three of the five senses (taste, touch, smell, sight, sound).

> _As you walk into the tavern, you are greeted by the smell of roasted lamb and stale beer. The fire crackles and conversations are quiet, you feel the warmth of the fire radiating from the hearth and there is a feeling of comfort and acceptance from the patrons._

Include the actions of other people around, particularly notable NPCs, such as those that might provide something to do, side quest or otherwise.

> _You see a brawny orc behind the bar nod in your direction as you walk into the tavern. A bard is hard at work to the left of the stage writing furiously in a book. A young woman is engaged in some quarrel with a man in the far corner of the room._

## Combat

### Combat Guidelines
- Combat should last around 3-5 rounds
- Should still be challenging
- For encounters, set a DC for sneak attacks or ambushes
  - Roll perception, less than X is surprised

### Theater of the Mind
If running theater of the mind, provide some additional elements in the room that players can use:

- A low hanging chandelier
- A large fireplace
- A conveniently (or inconveniently placed) obstacle

### Boss Fight Tips
When fighting bosses and big bads:

- Add minions for boss fights to enable the focus to be on the big bad and provide minions for action economy challenges
- Add a legendary action or two (summoning minions, lair actions, etc.)