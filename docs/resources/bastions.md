...
- The characters are on a long journey away from their Bastions. After the characters have been away for 7 days, you might say, "Time for a Bastion turn. Since you're not there, we'll assume you issue the Maintain order for your Bastions." Then roll for events as described in the "Events" section in this chapter.
...
Encourage players to create floor plans of their characters' Bastions, configuring its facilities as they see fit and using the same techniques you use to create dungeon maps (see "Dungeons" in 3 and "Maps" in 4).

In addition to basic and facilities (described later in this chapter), a Bastion can have the following features:
...
- **Defensive Walls** A character can add defensive walls around their Bastion. A defensive wall is 20 feet high and may include a walkway along its top, with a means to access it (such as a ladder or lift). Each 5-foot square of defensive wall takes 10 days to build and costs 250 GP. If a character's Bastion is completely enclosed by defensive walls and it comes under attack (see "Events" at the end of this chapter), reduce by 2 the number of dice rolled to determine how many Bastion Defenders are lost in the attack.

- **Doors and Windows** Each facility comes with one or more doors and shuttered windows, placed wherever the player sees fit. See "Doors" in 3 for kinds of doors to choose from, including locked doors, secret doors, and portcullises. These features are free.
...
A character's Bastion starts with two free basic facilities, which the character's player chooses from the Basic Facilities list below. One of the chosen facilities is Cramped, and the other is Roomy (see the Space table). A Bastion can have more than one of each basic facility.
...
Each special facility has a level. A character must be that level or higher to gain that facility. A special facility might also have a prerequisite the character must meet to gain that facility. For example, only a character who can use an Arcane Focus or a tool as a Focus can have an Study.


### Space


A special facility occupies a certain amount of space (see "Space"). A player can configure the squares of a special facility as desired on the Bastion's map. A special facility can be enlarged to grant additional benefits if its description says so.


### Hirelings


A special facility comes with one or more hirelings who work in the facility, maintain it, and execute Bastion orders there, as described in the next section. A player can assign names and personalities to hirelings in their character's Bastion using the same tools DMs use to NPCs (see 3).
...
All the Bastion's hirelings focus on maintaining the Bastion rather than executing orders in special facilities. Issuing this order prohibits other orders from being issued to the Bastion on the current Bastion turn. Each time the Maintain order is issued, the DM rolls once on the Events table at the end of this chapter. Bastion events are resolved before the next Bastion turn.


#### Recruit


Hirelings recruit creatures to the Bastion. These creatures might include one or more Bastion Defenders, whose main purpose is to defend the Bastion if it is attacked (see "Events" at the end of this chapter). The Bastion generates enough income to provide for the needs of its Bastion Defenders.
...
|5|Study|Ability to use an Focus or a tool as a Focus|Craft|
...
|5|Sanctuary|Ability to use a Symbol or Focus as a Focus|Craft|
...
|9|Sacristy|Ability to use a Symbol or Focus as a Focus|Craft|

|9|Scriptorium|None*|Craft|

|9|Stable|None|Trade|

|9|Circle|None|Recruit|

|9|Theater|None|Empower|

|9|Area|None|Empower|

|9|Room|None|Research|

|13|Archive|None|Research|

|13|Chamber|None|Empower|

|13|Menagerie|None|Recruit|

|13|Observatory|Ability to use a Focus|Empower|

|13|Pub|None|Research|

|13|Reliquary|Ability to use a Symbol or Focus as a Focus|Harvest|

|17|Demiplane|Ability to use an Focus or a tool as a Focus|Empower|

|17|Guildhall|Expertise in a skill|Recruit|

|17|Sanctum|Ability to use a Symbol or Focus as a Focus|Empower|

|17|Room|Fighting Style feature or Unarmored Defense feature|Recruit|
...
Players can use the Tracker sheet as a record of the facilities and other characteristics of their Bastions. Encourage your players to list their hirelings on this sheet and develop them more fully as NPCs on separate sheets (perhaps using the Tracker in 3). Similarly, the Bastion Tracker provides space to indicate the space of each special facility, but encourage players to draw maps of their Bastions (see "Map" earlier in this chapter).
...
If you seize the opportunity, you must pay 500 GP to cover costs. In return, your Bastion gains a sudden influx of recognition or attention, prompting the DM to roll again on the Events table (rerolling this result if it comes up again).
...
|1|The guest is an individual of great renown who stays for 7 days. At the end of their stay, the guest gives you a letter of recommendation (see "Prestige" in 3).|
...
|4|The guest is a [Attitude] monster, such as a brass dragon or a treant. If your Bastion is attacked while this monster is your guest, it defends your Bastion, and you lose no Bastion Defenders. The monster leaves after it defends your Bastion once or when you send it away.|
...
Your Bastion acquires an art object or a magic item determined by rolling on the table below and then rolling on the specified table in 7. How the Bastion acquires this treasure is up to you. It might represent an inheritance, a gift from a guest or an admirer, a theft, or a fortunate discovery. If you're in the Bastion, you can claim the treasure immediately; otherwise, it is placed in storage until you can claim it.

|1d100|Treasure|

|---|---|

|01–40|Roll on the 25 GP Objects table.|

|41–63|Roll on the 250 GP Objects table.|

|64–73|Roll on the 750 GP Objects table.|

|74–75|Roll on the 2,500 GP Objects table.|
...
- **Ruination** Drawing the Ruin card from the Things (as described in 7) instantly deprives a character of their Bastion. When such an event occurs, the player can decide what terrible fate befalls the Bastion. The Bastion might be sacked by enemies or destroyed by an earthquake, for example.

Regardless of how the Bastion falls, the player can work with the DM to establish a new Bastion and determine how it comes into being. Use the Acquisition table to determine how many special facilities come with it. The new Bastion also starts with two basic facilities (one Cramped and one Roomy) of the player's choice.
